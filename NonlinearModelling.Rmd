...
```
These are the test mean square errors for the linear, polynomial, cubic spline and smooth spline models respectively. The nonlinear models are all better predictors than the linear regression, and the polynomial regression is still a lot better than the cubic spline with a test MSE error of 260. The smooth spline model is also good, but the polynomial is still a bit better.
...
To compare our final model with the linear models, we also used the test error.

The linear models all have a test mean squared error between 650 and 900, the test mean squared error of this backwards selected GAM model is the following:
...
```
So, it is safe to conclude that this nonlinear model is a better model than all of the linear models, since it reduces the test errors by at least 63,1%, based on the best performing linear model, lasso.