...
cubic_names = paste("cubic.",4:10,sep = "")

for (i in 4:10){
  name = paste("cubic.", i, sep = "")
  cubic = lm(Cscore~bs(lpsa , df = i ),data=prostate[sample,])
  assign(name, cubic)
}
cubic_list <- lapply(cubic_names, function(x)get(x))
## Cross validation
mse = c()
for (model in cubic_list){
  MSE_model =mean((Cscore - predict(model, prostate))[-sample]^2)
  mse = c(mse,MSE_model)
}
min(mse)
```
...
## Smoothing Spline
```{r}
smooth_names = paste("smooth.",1:10,sep = "")

for (i in 2:10){
  name = paste("smooth.", i, sep = "")
  smoothspline = smooth.spline(train$lpsa, train$Cscore,  df= i )
  assign(name, smoothspline)
}
smooth_list <- lapply(smooth_names, function(x)get(x))
mse.smooth = c()
for (model in smooth_list){
  MSE_smooth =mean((Cscore - predict.smooth.spline(model, prostate))[-sample]^2)
  mse.smooth = c(mse.smooth,MSE_model)
}
min(mse.smooth)
##smoothspline_CV = smooth.spline( Cscore[sample],lpsa[sample], cv = TRUE)
##mean((Cscore-predict(smoothspline_CV, prostate))[-sample]^2)
>>>>>>> ff4f4a76499f9730c2865411fbd814e47ba160aa
```
